...
def testCount():
	...
	if c != 0:
		raise ValueError("After deletion, countPlayers should return zero. Got " + c)
...
def testPlayerStandingsSortedByWins():
	"""
	Test that matches are sorted properly.
	"""
	deleteMatches()
	deletePlayers()
	...
	wins = [row[2] for row in playerStandings()]

	if wins != [3, 2, 1, 0]:
		raise ValueError("Wins not reported in descending order. Got %s" % wins)

	print "11. After matches, players have updated standings reported by wins descending."
	deleteMatches()
	deletePlayers()
...
if __name__ == '__main__':
	...
	testReportMatches()
	testPairings()
	testPlayerStandingsSortedByWins()
